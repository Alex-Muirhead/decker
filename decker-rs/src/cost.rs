...
impl Cost {
    ...
    pub fn valid(&self) -> bool {
        !(self.coin.is_none() && self.potion.is_none() && self.debt.is_none())
    }

    pub fn get_string(&self) -> String {
        format!(
            "({},{},{})",
            self.coin.map(|i| i.to_string()).unwrap_or("".to_string()),
            self.potion.map(|i| i.to_string()).unwrap_or("".to_string()),
            self.debt.map(|i| i.to_string()).unwrap_or("".to_string())
        )
    }
...
impl CostUpto {
    pub fn new(matches_needed: i16, unmet_w: i16, met_w: i16, upper: i8) -> CostUpto {
        CostUpto {
...
impl CostTarget for CostUpto {
    ...
    fn add_votes(&self, current_costs: &CostSet, votes: &mut CostVotes) -> bool {
        let match_count = current_costs
            .iter()
            .filter(|coin| coin.is_coin_only())
            .filter_map(|cost| cost.get_coin())
            .filter(|&coin| coin <= self.limit)
            .count();

        let weight = if (match_count as i16) >= self.helper.matches_required {
        ...
        (match_count as i16) < self.helper.matches_required
    }
}

pub struct CostInSet {
    helper: CostTargetHelper,
    costs: CostSet,
}

impl CostInSet {
    pub fn new(matches_needed: i16, unmet_w: i16, met_w: i16, s: CostSet) -> CostInSet {
        CostInSet {
